...
<header class="w-full border-b border-border bg-background">
	<nav class="flex items-center justify-between px-6 py-4">
		<a href={resolve('/')} class="flex items-center gap-2 text-foreground font-semibold">
		...
		<ul class="flex items-center gap-8">
			{#each navItems as item (item.href)}
				<li>
					<a href={resolve(item.href)} class="text-sm text-muted-foreground hover:text-foreground transition-colors">
						{item.label}
					</a>
				</li>
			{/each}
		...
		<div class="flex items-center gap-3">
			<a href={resolve('/login')} class="px-4 py-2 text-sm text-foreground bg-secondary hover:bg-accent transition-colors">
				Login
			</a>

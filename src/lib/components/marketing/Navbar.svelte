	...
	const featureList: FeatureProps[] = [
		{
			icon: ChatCircleDots,
			title: 'Instant DMs',
			description: 'Fast, reliable messaging that stays in sync everywhere.'
		},
		{
			icon: Phone,
			title: 'Crystal-clear calls',
			description: 'Voice and video that just works, no dropped connections.'
		},
		{
			icon: UsersThree,
			title: 'Communities',
			description: 'Servers with channels, roles, and permissions that scale.'
		}
	];
...
<header class="w-[90%] md:w-[70%] lg:w-[75%] lg:max-w-screen-xl top-5 mx-auto sticky z-40 rounded-[15px] flex justify-between items-center p-2 bg-[#1a1720]/90 backdrop-blur-md shadow-md">
			...
			<Dialog.Portal>
				...
				<Dialog.Content class="fixed inset-y-0 left-0 z-50 flex w-72 flex-col justify-between rounded-tr-[15px] rounded-br-[15px] bg-[#1a1720] p-5">
					<div>
						...
						<div class="flex flex-col gap-1">
							{#each routeList as { href, label } (href)}
								<a
									href={href.startsWith('/') ? resolve(href as never) : href}
									onclick={() => (isOpen = false)}
									class="rounded-[10px] px-3 py-2 text-base text-zinc-300 hover:bg-white/5 hover:text-white"
								>
									{label}
								</a>
							{/each}
					...
					<div class="flex flex-col gap-3 pt-4">
						...
						<a
							href={resolve('/login')}
							class="rounded-full bg-white/5 px-4 py-2 text-center text-sm text-zinc-300 hover:bg-white/10 hover:text-white"
						>
							Log in
						</a>
						...
						<a
							...
							class="rounded-full bg-blue-500 px-4 py-2 text-center text-sm font-medium text-white hover:bg-blue-400"
						>
							Register
						</a>
					</div>
		...
		<DropdownMenu.Root>
			...
			<DropdownMenu.Portal>
				<DropdownMenu.Content
					class="z-50 w-[440px] rounded-[15px] bg-[#1a1720] p-3 shadow-xl"
					sideOffset={8}
				>
					<div class="flex flex-col gap-1">
						{#each featureList as { icon, title, description } (title)}
							...
							<DropdownMenu.Item class="flex items-start gap-3 rounded-[10px] p-3 outline-none data-[highlighted]:bg-blue-500/10">
								...
								<div>
									<p class="mb-1 text-sm font-semibold leading-none text-white">
										{title}
									</p>
	...
	<div class="hidden lg:flex items-center gap-2">
		...
		<a
			href={resolve('/login')}
			class="rounded-full px-4 py-2 text-sm text-zinc-300 hover:text-white"
		>
			Log in
		</a>
		...
		<a
			href={resolve('/register')}
			class="rounded-full bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-400"
		>
			Register
		</a>
	</div>
